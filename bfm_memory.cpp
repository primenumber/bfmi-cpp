...
int64_t memory::index_to_trueindex(int64_t index) {
  return index + offset;
}

int64_t memory::trueindex_to_index(int64_t true_index) {
...
int64_t memory::find_match_forward(int64_t index) {
  int64_t true_index = index_to_trueindex(index);
  int64_t count = 1;
...
int64_t memory::find_match_backward(int64_t index) {
  int64_t true_index = index_to_trueindex(index);
  int64_t count = 1;
...
int64_t memory::find_match(int64_t index) {
  switch ((*this)[index]) {
   case '[':
